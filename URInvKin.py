...
if UR10:
	...
	d5 = 0.1157
	d6 = 0.0922
	d = MATRIX([0.1273, 0, 0, 0.163941, 0.1157, 0.0922])
	a = MATRIX([0 ,-0.612 ,-0.5723 ,0 ,0 ,0])
...
def compute_dh_transform(n, th, c):
    """
...
def forward_kinematics(th, c):
    ...
    """
    A_1 = compute_dh_transform(1, th, c)
    A_2 = compute_dh_transform(2, th, c)
    A_3 = compute_dh_transform(3, th, c)
    A_4 = compute_dh_transform(4, th, c)
    A_5 = compute_dh_transform(5, th, c)
    A_6 = compute_dh_transform(6, th, c)
...
def invKine(desired_pos):
    ...
    for i in range(len(cl)):
        c = cl[i]
        T_10 = linalg.inv(compute_dh_transform(1, th, c))
    ...
    for i in range(len(cl)):
        c = cl[i]
        T_10 = linalg.inv(compute_dh_transform(1, th, c))
        T_65 = compute_dh_transform(6, th, c)
        T_54 = compute_dh_transform(5, th, c)
    ...
    for i in range(len(cl)):
        c = cl[i]
        T_10 = linalg.inv(compute_dh_transform(1, th, c))
        T_65 = linalg.inv(compute_dh_transform(6, th, c))
        T_54 = linalg.inv(compute_dh_transform(5, th, c))
        ...
        # Joint 4 (wrist 1)
        T_32 = linalg.inv(compute_dh_transform(3, th, c))
        T_21 = linalg.inv(compute_dh_transform(2, th, c))
        T_34 = T_32 * T_21 * T_14
